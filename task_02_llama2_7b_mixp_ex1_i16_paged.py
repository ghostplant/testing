...
if False: # using standard TorchOps, but GPU memory will overflow
  ...
  my_custom_fn = autort.export(ir="""
    w[M, K] = value_map_gpu[input1[M, K].unsigned_cast()] * page_err[M, K / 256] + page_avg[M, K / 256]
    my_result[M] +=! input0[K] * w[M, K]
  """, inputs=["input0=float32[K]", "input1=int16[M, K]", "page_avg=float32[M, K_GROUP]", "page_err=float32[M, K_GROUP]", "value_map_gpu=float32[L]"])

  def matmul_dequat(x, w, memory_out=None):
    x = x.view(-1)
    memory_out = memory_out if memory_out is not None else torch.empty([w.size(0)], dtype=x.dtype, device=x.device)
    return my_custom_fn(x, w,
      torch.zeros([w.size(0), w.size(1) // 256]).float().to(autort.device()),
      torch.ones([w.size(0), w.size(1) // 256]).float().to(autort.device()),
      value_map_gpu, memory_out.view(-1), out=5)
